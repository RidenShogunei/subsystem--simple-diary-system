    ...
    <div class="diary">
        <a-card class="detile">
            ...
            <div class="search" v-else-if="selectedKeys == 'search'">
                <a-textarea style="margin-top: 2%;" v-model:value="searchcontent" placeholder="在此输入您的搜查" />
                <a-date-picker style="width: 20%;margin-top: 2%;" v-model:value="time" />
                <a-button type="primary" style="width: 10%;margin-top: 2%;" @click="search">搜索</a-button>
            </div>
            <div class="display" v-else-if="selectedKeys == 'display'">
...
const submit = () => {
    instance.appContext.config.globalProperties.$axios.post('/api/submit', { data: content.values })
    .then(response => {
      // 处理响应数据
...
const search=()=>{
    instance.appContext.config.globalProperties.$axios.post('/api/search', { data: searchcontent.values,date:time.values })
    .then(response => {
      // 处理响应数据
      console.log(response.data);
    })
